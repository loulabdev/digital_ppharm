...
const Spine = ({ height = "100%" }: { height?: string | number }) => (
  <div className="book-spine-hide" style={{ width: "clamp(16px,2.2vw,24px)", flexShrink: 0, position: "relative", zIndex: 10, backgroundColor: C.page1, height }}>
    ...
    <div style={{ position: "absolute", bottom: 3, left: "50%", transform: "translateX(-50%)", width: 20, height: 20, zIndex: 13 }}>
      <div style={{ position: "absolute", left: "50%", top: "50%", transform: "translate(-50%,-50%)", width: 6, height: 6, borderRadius: "50%", background: "radial-gradient(circle, #5aaa60 30%, #2e6a38 100%)", boxShadow: "0 -6px 0 1.5px #3a8848, 0 6px 0 1.5px #3a8848, -6px 0 0 1.5px #3a8848, 6px 0 0 1.5px #3a8848" }} />
      <div style={{ position: "absolute", left: "50%", top: "50%", transform: "translate(-50%,-50%)", width: 5, height: 5, borderRadius: "50%", background: "radial-gradient(circle, #7acc80 30%, #3a7a48 100%)", zIndex: 2 }} />
    </div>
  </div>
);
...
const PrescriptionView: React.FC<Props> = ({ data, onReset, onBookmarksChange }) => {
  ...
  const handleCardClick = useCallback((book: RecommendedBook) => {
    ...
    if (openBookKey) { cancelType(); setClosingKey(openBookKey); setTimeout(() => setClosingKey(null), 300); }
    setOpenBookKey(key); setPanelUp(false);
    setTimeout(() => { setPanelUp(true); detailRef.current?.scrollIntoView({ behavior: "smooth", block: "nearest" }); }, 30);
    const hp = book.healing_point || "";
    if (hp) setTimeout(() => typeText(setQuoteText, setQuoteDone, hp, 55), 220);
    if (!coverUrls[key] && coverUrls[key] !== null) { /* 이미 로드됨 */ }
  ...
  }, [openBookKey, cancelType, typeText, coverUrls, bookDescriptions, closePanel]);

  const handleNextBook = useCallback(() => {
    if (!openBookKey) return;
    const idx = data.recommended_books.findIndex(b => getBookKey(b) === openBookKey);
    const next = data.recommended_books[idx + 1];
    if (next) handleCardClick(next);
  }, [openBookKey, data.recommended_books, handleCardClick]);
  ...
  const searchLibraries = async (book: RecommendedBook, rk: string) => {
    setLoadingBooks(p => ({ ...p, [rk]: true }));
    setLibraryErrors(p => ({ ...p, [rk]: null }));
    try {
  ...
  const handleFindLatestEdition = async (book: RecommendedBook) => {
    ...
    try {
      ...
      const isSpecialEdition = (title?: string) => title ? SPECIAL_EDITION_KEYWORDS.some(kw => title.includes(kw)) : false;
      ...
      for (const ed of lr.allEditions) { if (ed.isbn13 && !isSpecialEdition(ed.title)) isbnSet.add(ed.isbn13); }
  ...
  const handleSaveMemo = () => {
    try {
      const key = `rx_memo_${data.emotional_analysis.detected_emotion}_${Date.now()}`;
  ...
  const AudiobookBadges = ({ book }: { book: RecommendedBook }) => {
    ...
    const audioPlatforms = (book.tags || []).filter(t => t in AUDIO_PLATFORMS).map(t => AUDIO_PLATFORMS[t]);
  ...
  const LibraryBlock = ({ book }: { book: RecommendedBook }) => {
    ...
    const filtered = (isLatestSearch || showAll) ? allLibraries : allLibraries.filter(inRegion);
    const sorted   = [...filtered].sort((a, b) => {
      if (typeof a.distanceKm === "number" && typeof b.distanceKm === "number") return a.distanceKm - b.distanceKm;
      return typeof a.distanceKm === "number" ? -1 : typeof b.distanceKm === "number" ? 1 : 0;
    });
    ...
    return (
      <>
        {!hasSearched && !isLoading && <p style={{ ...SE, fontSize: 10, color: C.ink3, textAlign: "center", padding: "4px 0" }}>추가 검색에서 도서관 조회를 선택하세요</p>}
        {isLoading && <p style={{ ...SE, fontSize: 10, color: C.ink3, textAlign: "center", padding: "4px 0" }}>조회 중...</p>}
        {libraryError && !isLoading && <div style={{ ...SE, fontSize: 10, color: "#c05030", padding: "3px 7px", background: "rgba(255,200,180,0.3)", borderRadius: 4, marginBottom: 4 }}>{libraryError}</div>}
            ...
            {isLibExp && (
              <>
                <div style={{ padding: "5px 8px 4px", borderTop: `1px dotted rgba(110,84,40,0.18)`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <span style={{ ...SE, fontSize: 9, color: C.ink3 }}>{isLatestSearch ? "거리순 상위 10곳 (전국)" : userRegionName ? `${userRegionName} 지역 내` : "전체"}</span>
        ...
        <div style={{ display: "flex", borderRadius: 2, overflow: "hidden", border: "3px solid #1a3a24", boxShadow: "0 16px 40px rgba(0,0,0,0.60),0 4px 10px rgba(0,0,0,0.30)", position: "relative" }}>
          ...
          <div style={{ flex: 1, padding: "20px 16px 60px 20px", position: "relative", ...paperStyle(C.page1) }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                <button type="button" onClick={onReset} style={{ ...SE, fontSize: 9, color: C.ink3, background: "none", border: "none", cursor: "pointer", borderBottom: `1px dotted rgba(110,84,40,0.36)`, padding: "0 0 1px" }}>디지털 종이약국</button>
                ...
                return (
                  <>
                    <div style={{ maxHeight: isLong && !isExp ? 68 : (isExp ? 160 : "none"), overflowY: isExp ? "auto" : "hidden", transition: "max-height 0.3s ease" }}>
                      <p style={{ fontFamily: "'Noto Serif KR', serif", fontSize: 11, color: "#3c2e14", lineHeight: 1.85, wordBreak: "keep-all" }}>{desc}</p>
                    </div>
                    {isLong && (
          ...
          <div style={{ flex: 1, padding: "20px 20px 60px 16px", position: "relative", ...paperStyle(C.page2) }}>
            ...
            <div style={{ border: `1px solid ${C.bdr}`, borderRadius: 4, padding: "7px 9px", marginBottom: 6, background: "rgba(255,255,255,0.5)" }}>
              ...
              {book.music_keyword && (
                <div style={{ display: "flex", gap: 5, marginTop: 4, paddingTop: 4, borderTop: `1px dotted rgba(110,84,40,0.18)` }}>
                  <span style={{ ...SE, fontSize: 10, color: C.ink3, flexShrink: 0 }}>🎵</span>
                  <span style={{ ...GB, fontSize: 10.5, color: C.ink, lineHeight: 1.65 }}>
                    <a href={`https://www.youtube.com/results?search_query=${encodeURIComponent(book.music_keyword)}`} target="_blank" rel="noopener noreferrer"
    ...
    return (
      <div ref={detailRef} style={{ borderRadius: 16, overflow: "visible", boxShadow: "0 16px 40px rgba(0,0,0,0.55),0 4px 12px rgba(0,0,0,0.35)", border: "3px solid #1a3a24", maxWidth: 360, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "11px 15px 9px", background: C.page1, borderBottom: `1px solid rgba(180,160,120,0.2)` }}>
          <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
            <span style={{ ...SE, fontSize: 10, color: "#222", letterSpacing: "1px" }}>디지털 종이약국</span>
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 11 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 3 }}>
                {[book.genre, ...(book.tags || [])].filter(Boolean).slice(0, 3).map((t, i) => (
                  <span key={i} style={{ ...SE, fontSize: 9.5, padding: "1px 6px", border: `1px solid rgba(110,84,40,0.3)`, borderRadius: 8, color: "#6a5030", background: "rgba(255,248,220,0.7)" }}>{t}</span>
                ))}
              </div>
            </div>
          </div>
          <SH label="책  소개" />
          <div style={{ background: "rgba(175,155,110,0.16)", borderRadius: 4, padding: "7px 9px", marginBottom: 6 }}>
            {(() => {
              const naverDesc = bookDescriptions[key] || "";
              const desc = naverDesc || book.why_this_book || "";
              if (!desc) return <p style={{ ...SE, fontSize: 10, color: C.ink3, opacity: 0.5 }}>· · ·</p>;
              ...
              return (
                <>
                  <div style={{ maxHeight: isLong && !isExp ? 60 : "none", overflowY: "hidden", transition: "max-height 0.3s ease" }}>
                    <p style={{ fontFamily: "'Noto Serif KR', serif", fontSize: 10.5, color: "#3c2e14", lineHeight: 1.8, wordBreak: "keep-all" }}>{desc}</p>
                  </div>
                  {isLong && (
                    <button type="button" onClick={() => setDescExpanded(p => ({ ...p, [key]: !p[key] }))}
                      style={{ ...SE, fontSize: 9, color: C.ink2, background: "none", border: "none", cursor: "pointer", marginTop: 3, padding: 0, opacity: 0.75 }}>
                      {isExp ? "접기 ▲" : "더보기 ▼"}
                    </button>
                  )}
                </>
        ...
        <div style={{ ...paperStyle(C.page2), padding: "20px 15px 14px" }}>
          ...
          <div style={{ border: `1px solid ${C.bdr}`, borderRadius: 4, padding: "7px 9px", marginBottom: 5, background: "rgba(255,255,255,0.5)" }}>
            ...
            {book.music_keyword && (
              <div style={{ display: "flex", gap: 4, marginTop: 4, paddingTop: 4, borderTop: `1px dotted rgba(110,84,40,0.18)` }}>
  ...
  const BookCard = ({ book, index }: { book: RecommendedBook; index: number }) => {
    ...
    const isBookmarked = bookmarkedKeys.has(key);
    return (
      <button type="button" onClick={() => handleCardClick(book)}
        style={{ background: "transparent", border: "none", padding: 0, cursor: "pointer", position: "relative", textAlign: "left", WebkitTapHighlightColor: "transparent", display: "flex", flexDirection: "column", alignItems: "center", transition: "transform 0.18s ease", width: "100%", maxWidth: isTablet ? 132 : 105 }}
        onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = "translateY(-3px)"; }}
        onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = "translateY(0)"; }}
      >
        <div style={{ width: "100%", aspectRatio: "2/3", position: "relative", overflow: "hidden", borderRadius: 6, boxShadow: isOpen ? "0 6px 18px rgba(0,0,0,0.38)" : "2px 4px 12px rgba(0,0,0,0.22)", transition: "box-shadow 0.2s ease", transform: "perspective(400px) rotateY(-4deg)", transformOrigin: "left center" }}>
          {coverUrl
            ? <img src={coverUrl} alt={book.title} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }} />
            : <canvas ref={el => { if (el) { const s = GHIBLI_SCENES[index % GHIBLI_SCENES.length]; drawGhibli(el, s.scene, s.bg1, s.bg2); } }} width={120} height={180} style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }} />}
          ...
          <div style={{ position: "absolute", top: 5, right: 5, zIndex: 5, background: "rgba(0,0,0,0.42)", borderRadius: 4, padding: "1px 5px", ...SE, fontSize: 8, color: "rgba(255,255,255,0.9)" }}>{String(index + 1).padStart(2, "0")}</div>
          {isBookmarked && <div style={{ position: "absolute", top: 5, left: 5, zIndex: 5, width: 6, height: 6, borderRadius: "50%", background: C.seal }} />}
          {isOpen && <div style={{ position: "absolute", inset: 0, border: `2px solid ${C.cover1}`, borderRadius: 6, zIndex: 6 }} />}
        </div>
        <div style={{ marginTop: 6, width: "100%", ...SE, fontSize: 9.5, color: C.ink, lineHeight: 1.3, textAlign: "center", overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" as const }}>
          {book.title}
        </div>
      </button>
    );
      ...
      <div className="animate-fade-in" style={{ maxWidth: 720, margin: "0 auto", padding: "0 16px 80px" }}>
        ...
        {data.quiz_answers && (
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", padding: "14px 0 6px", alignItems: "center" }}>
            ...
            {[
              MOOD_LABEL[data.quiz_answers.mood],
              CONCERN_LABEL[data.quiz_answers.concern],
              READ_STYLE_LABEL[data.quiz_answers.readStyle],
            ].map((label, i) => (
              <span key={i} style={{ ...SE, fontSize: 10, padding: "2px 10px", borderRadius: 12, background: "rgba(26,58,32,0.08)", border: `1px solid rgba(26,58,32,0.18)`, color: C.greenDark }}>
                {label}
              </span>
            ))}
          </div>
        ...
        <section style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(data.recommended_books.length, 4)}, minmax(0, 1fr))`, gap: 6, marginBottom: 16, justifyItems: "center" }}>
        ...
        {/* ── 추가 제안 ── */}
        <section style={{ background: C.page1, border: `1px solid ${C.bdr}`, padding: "16px 20px", borderRadius: 12, marginBottom: 16 }}>
